...
from result_parsers.sandbox import MyBox
import re
import numpy as np
...
class Executor:
    """
    A sandbox that can run python code.
    """
    def __init__(self, config=None):
        self.config = config
        self.sandbox = MyBox(config)
    ...
    def process_single_row(self, sample):
        try:
            ...
            if sample.df is not None:
                code_result = run_test_with_timeout(self.sandbox, {'df': sample.df}, last_code)
            else:
                code_result = run_test_with_timeout(self.sandbox, {}, last_code)
